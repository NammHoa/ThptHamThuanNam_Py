...
if (isset($_SESSION['import_msg'])) {
    ...
    if ($msg === 'cleared')     { $resultMsg = "Đã xoá toàn bộ danh sách.";           $resultType = 'danger';  }
...
<head>
  ...
  <style>
    ...
    .btn-danger-outline { display:inline-flex; align-items:center; gap:6px; padding:8px 16px; background:transparent; color:#dc3545; border:1.5px solid #dc3545; border-radius:6px; font-size:13px; cursor:pointer; text-decoration:none; transition:all 0.2s; white-space:nowrap; }
    ...
    .upload-section h3 { color:#004080; margin:0 0 5px 0; font-size:16px; display:flex; align-items:center; gap:8px; }
    ...
    .btn-upload { display:inline-flex; align-items:center; gap:8px; padding:9px 24px; background:linear-gradient(135deg,#28a745,#20c050); color:#fff; border:none; border-radius:8px; font-size:14px; font-weight:bold; cursor:pointer; }
    .btn-upload:hover { background:linear-gradient(135deg,#218838,#1aab42); }

    .result-box { display:flex; align-items:center; gap:10px; padding:14px 18px; border-radius:8px; margin-bottom:15px; font-size:14px; line-height:1.7; }
    ...
    .btn-reset { display:inline-flex; align-items:center; gap:6px; padding:9px 16px; background:#e0e0e0; color:#555; border:none; border-radius:8px; font-size:14px; cursor:pointer; text-decoration:none; }
    ...
    .btn-edit-row { display:inline-flex; align-items:center; gap:4px; padding:5px 10px; background:transparent; color:#007bff; border:1px solid #007bff; border-radius:5px; font-size:12px; cursor:pointer; transition:all 0.2s; white-space:nowrap; margin-right:4px; }
    .btn-edit-row:hover { background:#007bff; color:#fff; }
    .btn-del-row { display:inline-flex; align-items:center; gap:4px; padding:5px 10px; background:transparent; color:#dc3545; border:1px solid #dc3545; border-radius:5px; font-size:12px; cursor:pointer; transition:all 0.2s; white-space:nowrap; text-decoration:none; }
    ...
    .modal-title { font-size:17px; font-weight:bold; color:#004080; margin:0; display:flex; align-items:center; gap:8px; }
    ...
    .btn-cancel { display:inline-flex; align-items:center; gap:6px; padding:9px 18px; background:#e0e0e0; color:#555; border:none; border-radius:7px; font-size:14px; cursor:pointer; }
    .btn-save   { display:inline-flex; align-items:center; gap:6px; padding:9px 22px; background:linear-gradient(135deg,#28a745,#20c050); color:#fff; border:none; border-radius:7px; font-size:14px; font-weight:bold; cursor:pointer; }
    ...
    .list-card-actions .btn-edit-row, .list-card-actions .btn-del-row { flex:1; justify-content:center; padding:7px; }
...
<main>
  ...
  <?php if ($resultMsg): ?>
    <div class="result-box result-<?= $resultType ?>" id="result-msg">
      <i class="ti ti-<?= $resultType === 'success' ? 'circle-check' : 'alert-triangle' ?>"></i>
      <span><?= $resultMsg ?></span>
    </div>
    <script>
      setTimeout(() => {
  ...
  <div class="stats-bar">
    ...
    <?php if ($total > 0): ?>
      <a href="?clear=1" class="btn-danger-outline"
         onclick="return confirm('Xác nhận xoá TOÀN BỘ danh sách?')">
        <i class="ti ti-trash"></i> Xoá toàn bộ
      </a>
    <?php endif; ?>
  </div>
  ...
  <div class="upload-section">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <button type="submit" class="btn-upload">
        <i class="ti ti-upload"></i> Import dữ liệu
      </button>
    </form>
  </div>
  ...
  <?php if ($total > 0 || $search !== ''): ?>
    ...
    <form method="GET" class="search-bar">
      ...
      <?php if ($search !== ''): ?>
        <a href="import_trungtuyen.php" class="btn-reset">
            ...
            <tr>
              ...
              <td style="text-align:center;">
                ...
                <a href="?delete_id=<?= $row['id'] ?><?= $search ? '&search='.urlencode($search) : '' ?>"
                   class="btn-del-row"
                   onclick="return confirm('Xác nhận xoá học sinh này?')">
      ...
      <?php $listResult->data_seek(0); $stt = $offset+1; while ($row = $listResult->fetch_assoc()): ?>
        <div class="list-card">
          ...
          <div class="list-card-info">
            ...
            <i class="ti ti-cake"></i> <?= htmlspecialchars($row['ngay_sinh']) ?>
          </div>
